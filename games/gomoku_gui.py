...
class MuZeroConfig:
    def __init__(self):
        # fmt: off More information is available here:
        # https://github.com/werner-duvaud/muzero-general/wiki/Hyperparameter-Optimization

        self.seed = 0  # Seed for numpy, torch and the game
        self.max_num_gpus = None  # Fix the maximum number of GPUs to use. It's usually faster to use a single GPU (
        # set it to 1) if it has enough memory. None will use every GPUs available

        ### Game
        self.observation_shape = (3, 11,
                                  11)  # Dimensions of the game observation, must be 3 (channel, height, width). For
        # a 1D array, please reshape it to (1, 1, length of array)
