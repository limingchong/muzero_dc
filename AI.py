...
class AI:
    ...
    def get_action(self, game_history):
        ...
        action = self.select_action(
            ...
            0
            if not 0
               or len(game_history.action_history) < 0
            else 0,
        )

        return action, root

    def update_history(self, action, root, game_history, reward, to_play):
        game_history.store_search_statistics(root, self.config.action_space)

        # Next batch
        game_history.action_history.append(action)
        game_history.observation_history.append(self.game.get_observation())
